...
const FALLBACK_MESSAGE = 'Ocorreu um erro inesperado ao processar o plano de ensino.'
...
function statusMessage(status?: number): string | null {
  ...
  if (status === 404) return 'Plano de ensino não encontrado.'
  if (status !== undefined && status >= 500) return 'Erro interno no servidor. Tente novamente em instantes.'
  return null
}

export function extractTeachingPlanErrors(error: unknown): string[] {
